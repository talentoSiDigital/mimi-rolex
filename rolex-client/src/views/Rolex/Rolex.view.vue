            ...
            <div class="flex flex-col justify-center items-center w-full">
                <div id="rolex-container" class="grid grid-cols-1 place-items-center md:grid-cols-3 w-8/12">
                    <div v-for="(item,index) in state" id="card" class="w-full border border-transparent hover:border-gray-700" :key="index">
                        <router-link :to="{name:'rolex-coleccion', params: {id: item.idName}}" v-if="item.nombre != 'MUJERES' && item.nombre != 'ORO' && item.nombre != 'HOMBRES'" >
            ...
            <section v-if="isReady" class="flex  mt-14 justify-center items-center">
                <div class="flex flex-col w-9/12">
                    ...
                    <div class="flex justify-center items-center gap-4">
                        <router-link :to="{name:'rolex-coleccion',params:{id:'HOMBRES'}}">
                            <img :src="state[14].file" :alt="state[14].watch">
                            <h2 class="uppercase font-bold mt-4">Relojes rolex para {{ state[15].nombre }}</h2>
                        </router-link>
                        <router-link :to="{name:'rolex-coleccion',params:{id:'MUJERES'}}">
                            <img :src="state[15].file" :alt="state[15].watch">
                            <h2 class="uppercase font-bold mt-4">Relojes rolex para {{ state[15].nombre }}</h2>
                        </router-link >
                    </div>
                    <router-link :to="{name:'rolex-coleccion', params: {id: 'Oro'}}" class="w-full mt-8">
